<template>
	<view>
		<view class="search_desc">
		    <span class="search_desc_text">搜索关键2词：<span class="color_blue">{{searchKeywords}}</span></span>
		...
		<view class="video_box">
			<view class="video_img"  v-for="(image, index) in imgList" :key="index">
				<img :src="image.pic" alt="" @tap="goDetail(image.id)">
...
<script>
	import {chooseNav,searchResult} from '@/config/http.js';
	export default {
		data() {
			return {
				imgList:[],
		...
		onLoad: function (option) { //option为object类型，会序列化上个页面传递的参数
			this.searchKeywords = option.name;
			this.type = option.type;
		    console.log(option,231231); //打印出上个页面传递的参数。
			this.getVideoList()
		},
		...
		methods: {
			...
			getData() {
				searchResult({
					...
					this.searchCount = Object.keys(this.imgList).length
					if(Object.keys(this.imgList).length==0){
						this.noSearch = true
					}
					console.log(res.data.data.list);
				 })
